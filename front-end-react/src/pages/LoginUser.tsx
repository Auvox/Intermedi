...
export default function LoginUser({ initialMode = "login" }: { initialMode?: Mode }) {
  ...
  function changeMode(next: Mode) { setMode(next); setMessage(""); setVisible(false); }
  function submit(event: FormEvent<HTMLFormElement>) {
    ...
    // No authentication endpoints exist yet. Never simulate an authenticated session.
    setMessage(mode === "cadastro" ? "O cadastro online ainda não está disponível. Nenhuma conta foi criada. Entre em contato com o responsável pela plataforma." : "O acesso à plataforma ainda não está disponível. Tente novamente mais tarde ou fale com o responsável pela sua farmácia.");
  }
    ...
    <section className="auth-main" aria-label="Acesso ao Intermedi">
      ...
      <div className={`auth-content${selected && mode === "cadastro" ? " auth-register" : ""}`}>
        ...
        {!selected ? <>
          ...
          <button className="auth-back" onClick={() => { setRole(null); setMessage(""); setVisible(false); }}><Icon name="back" /> Trocar perfil</button>
          ...
          <form key={`${role}-${mode}`} className="auth-form" onSubmit={submit}>
            ...
            <button className="auth-submit" type="submit">{mode === "login" ? "Entrar na minha conta" : "Criar minha conta"}<Icon name="arrow" /></button>
          </form>
